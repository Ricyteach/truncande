...
@click.argument("ofile", type=click.Path(exists=False, dir_okay=False, writable=True), required=False)
@click.pass_context
def main(ctx, ifile, ofile=""):
    ifile = pathlib.Path(ifile)
    ofile = pathlib.Path(ofile) if ofile else ifile.parent/(ifile.stem + " truncated.txt")
